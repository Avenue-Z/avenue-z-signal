...
import { Settings, Send, ChevronDown, ChevronRight, ExternalLink, Check, X, Loader2, Terminal, MessageSquare, Globe, Search, Zap, TrendingUp } from "lucide-react";
...
.gt{background:linear-gradient(135deg,#FFFC60,#60FF80,#60FDFF,#39A0FF,#6034FF);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;}
.gb{background:linear-gradient(135deg,#FFFC60,#60FF80,#60FDFF,#39A0FF,#6034FF);}
.shimmer{background:linear-gradient(90deg,#272727 25%,#333 50%,#272727 75%);background-size:200% 100%;animation:sh 1.5s infinite;}
@keyframes sh{0%{background-position:200% 0}100%{background-position:-200% 0}}
.blink{animation:bl 1s step-end infinite;}
@keyframes bl{50%{opacity:0}}
.fi{animation:fi 0.4s ease-in;}
...
function ChkItem({ label, passed, detail }) {
  return (
...
function SettingsModal({ apiKey, onSave, onClose }) {
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000000dd",zIndex:100,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={onClose}>
      <div style={{background:"#1e1e1e",border:"1px solid #ffffff18",borderRadius:18,padding:28,width:420}} onClick={e=>e.stopPropagation()}>
        <div style={{color:"#fff",fontWeight:800,fontSize:16,marginBottom:6}}>API Configuration</div>
        <p style={{color:"#666",fontSize:13,marginBottom:20,lineHeight:1.5}}>Firecrawl key for live scraping. Pre-loaded from your session.</p>
        <label style={{color:"#666",fontSize:11,fontWeight:700,display:"block",marginBottom:6,letterSpacing:"0.08em"}}>FIRECRAWL API KEY</label>
        <input value={val} onChange={e=>setVal(e.target.value)} placeholder="fc-xxxxxxxxxxxxxxxx"
          style={{width:"100%",background:"#111",border:"1px solid #ffffff18",borderRadius:10,padding:"10px 14px",color:"#fff",fontSize:13,outline:"none",marginBottom:16}}/>
        <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
          <button onClick={onClose} style={{background:"#111",border:"1px solid #ffffff18",color:"#666",padding:"9px 20px",borderRadius:99,fontSize:13,cursor:"pointer",fontWeight:700}}>Cancel</button>
          <button onClick={()=>{onSave(val);onClose();}} className="gb" style={{color:"#000",fontWeight:800,padding:"9px 22px",borderRadius:99,fontSize:13,cursor:"pointer",border:"none"}}>Save</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  const [fcKey,setFcKey]=useState("fc-c3666768a30842c5ba37bd4325b1df8d");
...
Then return ONLY valid JSON (absolutely no markdown fences, no explanation):
{
  ...
  "contentQuality": "brief honest assessment of their content quality and depth"
      ...
      let ps=null;
      try{
        const psApiUrl=`https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=${encodeURIComponent(u)}&category=performance&category=accessibility&category=best-practices&category=seo&strategy=mobile`;
        const proxy=`https://api.allorigins.win/get?url=${encodeURIComponent(psApiUrl)}`;
        const r=await fetch(proxy);
        const j=await r.json();
        const pj=JSON.parse(j.contents);
      ...
      <div style={{background:"#0a0a0a",height:"100vh",color:"#fff",display:"flex",flexDirection:"column",overflow:"hidden"}}>
        ...
        <div style={{background:"#000",borderBottom:"1px solid #ffffff12",padding:"0 20px",height:48,display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <button onClick={()=>setShowSettings(true)} style={{background:"#111",border:"1px solid #ffffff15",borderRadius:8,padding:"6px 10px",cursor:"pointer",display:"flex",alignItems:"center",gap:6,color:"#555",marginLeft:4}}>
              <Settings size={13}/>
              <div style={{width:6,height:6,borderRadius:"50%",background:fcKey?"#60FF80":"#FF6060"}}/>
            </button>
          </div>
        </div>
        ...
        {showSettings&&<SettingsModal apiKey={fcKey} onSave={setFcKey} onClose={()=>setShowSettings(false)}/>}
      </div>
    </>
  );
}
